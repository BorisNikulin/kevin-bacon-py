...
def main():
    ...
    if not path.isfile(dijkstra_table_serialized_filename)\
        ...
        with open(dijkstra_table_serialized_filename, 'wb') as f:
            dijkstra_table.generate_table(graph, graph.get_adjacent, lambda e: 1)
            pickle.dump(dijkstra_table, f, 4)

        with open(dijkstra_table_weighted_serialized_filename, 'wb') as f:
            dijkstra_table_weighted.generate_table(graph, graph.get_adjacent, Movie.get_weight)
    ...
    for end in ends:
        print('Unweighted: ' +
              search_to_string(dijkstra_table.start, dijkstra_table.get_path_to(Actor(end))))
        print('Weighted:   ' +
              search_to_string(dijkstra_table_weighted.start, dijkstra_table_weighted.get_path_to(Actor(end))))
    print()
    ...
    for v in dijkstra_table_weighted:
        bacon_counts_weighted[path_to_bacon_number(dijkstra_table_weighted.get_path_to(v))] += 1
